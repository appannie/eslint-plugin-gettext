...
function isRegisteredTranslatable(context, literal, translationContext) {
    const poFilename = context.settings.poFilename;
    const translations = getTranslations(poFilename, translationContext);
    return (
        Object.keys(translations).findIndex(i => {
            return i === literal;
        }) !== -1
    );
}
...
module.exports = {
    ...
    isRegisteredTranslatable: isRegisteredTranslatable,
    isRegisteredPlural: isRegisteredPlural,
};
